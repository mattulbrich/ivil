(* created : Thu Mar 14 21:32:54 CET 2013 - Alpha-3 *)
#
# This file was generated by PSEUDO
# which is copyright (C) 2009 Universitaet Karlsruhe, Germany
#    written by Mattias Ulbrich
#

include "$bytecode.p"
include "$intRange.p"
include "declarations.p"

(* program variables *)
function
   ref _array assignable (* local variable '1' *)
   ref pre_array assignable (* method parameter, prestate *)
   int _from assignable (* method parameter / ... *)
   int pre_from assignable (* method parameter, prestate *)
   heap ho1 assignable (* Heap snapshot for loop invariant *)
   int st_0_int assignable (* stack variable *)
   int _i assignable (* local variable '3' *)
   ref st_1_ref assignable (* stack variable *)
   int st_1_int assignable (* stack variable *)
   bool branchCond assignable (* branching condition *)
   ref st_0_ref assignable (* stack variable *)
   bool st_0_bool assignable (* stack variable *)

(* the translated Java program *)
program Java source "./BFS.jspec"
   sourceline 8
    assume !_array = null ; "Assume precondition"
    assume wellformed(h) ; "assumption guaranteed by Java"
    assume !_this = null ; "assumption guaranteed by Java"
    assume h[_this, created] ; "assumption guaranteed by Java"
    assume typeof(_this) = C_BFS ; "assumption of the runtime type"
    assume subtype(typeof(_array), array_boolean) ; "Typing of argument"
    pre_h := h || pre_array := _array || pre_from := _from ; "Recording the prestate"
    decrBase := 0 ; "Save the decrease (variant) base"
  Label1:
   sourceline 17
    (* LDC + INVOKESTATIC - Specification injection *)
    ho1 := h
    (* spec statement deferred till after looping point *)
  Label2:
   sourceline 23
    (* ILOAD *)
    st_0_int := _from
    (* ISTORE *)
    _i := st_0_int
  Label3:
    (* deferred specification statement *)
    skip LOOPINV, {ho := ho1}((from <= _i & _i <= arrlen(_array) &
	     (\forall j; 0<=j&j<_i -> !h[_array, idxBool(j)])) & modHeap(h, ho, {h:=ho}(emptyset))), arrlen(_array) - _i
    (* ILOAD *)
    st_0_int := _i
    (* ALOAD *)
    st_1_ref := _array
    (* ARRAYLENGTH *)
    assert !st_1_ref = null ; "non-null array reference"
    st_1_int := arrlen(st_1_ref)
    (* IF_ICMPGE *)
    branchCond := st_0_int >= st_1_int
    goto Label4, Label5
  Label4:
    assume branchCond
    goto Label6
  Label5:
    assume !branchCond
  Label7:
   sourceline 24
    (* ALOAD *)
    st_0_ref := _array
    (* ILOAD *)
    st_1_int := _i
    (* BALOAD *)
    assert !st_0_ref = null ; "non-null array access"
    assert st_1_int >= 0 & st_1_int < arrlen(st_0_ref) ; "array index in bounds"
    st_0_bool := h[st_0_ref, idxBool(st_1_int)] ; "read value from array"
    (* IFEQ *)
    branchCond := !st_0_bool
    goto Label8, Label9
  Label8:
    assume branchCond
    goto Label10
  Label9:
    assume !branchCond
  Label11:
   sourceline 25
    (* ILOAD *)
    st_0_int := _i
    (* IRETURN *)
    exc := null
    resInt := st_0_int
    goto LabelEnd
  Label10:
   sourceline 23
    (* IINC *)
    _i := _i + 1
    (* GOTO *)
    goto Label3
  Label6:
   sourceline 28
    (* ICONST_M1 *)
    st_0_int := -1
    (* IRETURN *)
    exc := null
    resInt := st_0_int
    goto LabelEnd
  Label12:
  LabelEnd:
    assert exc = null ; "No exception has been thrown"
   sourceline 9
    assert -1 = resInt | _from <= resInt & resInt < arrlen(_array) ; "Assert postcondition"
   sourceline 10
    assert resInt >= _from -> h[_array, idxBool(resInt)] &
                     (\forall i; _from<=i & i<resInt -> !h[_array, idxBool(i)]) ; "Assert postcondition"
   sourceline 12
    assert resInt = -1 -> (\forall i; 0<=i & i<arrlen(_array) -> 
                                  !h[_array,idxBool(i)]) ; "Assert postcondition"
   sourceline 14
    assert modHeap(h, pre_h, {h:=pre_h}(emptyset))
(* end of program *)

