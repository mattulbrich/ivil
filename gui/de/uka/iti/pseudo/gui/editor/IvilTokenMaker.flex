...
|       {SchemaTypeIdentifier}                          { addToken(Token.VARIABLE); }
	...
	/* Comment literals. */
	{MLCBegin}					{ start = zzMarkedPos-2; yybegin(MLC);commentLevel++; System.out.println(start + " increments to " + commentLevel);}
...
<MLC> {
	...
	{MLCEnd}    { commentLevel --;
	System.out.println(start + " decrements to " + commentLevel);
	              if(commentLevel == 0) {
	                yybegin(YYINITIAL); 
	...
	\n |
        <<EOF>>     { addToken(start,zzStartRead-1, Token.COMMENT_MULTILINE);
                      ...
                      System.out.println(start + " ends with level " + leveledState);
