...
public class EqualsVisitor implements GenericVisitor<Boolean, Node> {
    ...
    @Override
    public Boolean visit(MethodSpecification n1, Node arg) {
        MethodSpecification n2 = (MethodSpecification) arg;
        for (int i = 0; i < n1.getContracts().size(); i++)
            if (!nodeEquals(n1.getContracts().get(i), n2.getContracts().get(i)))
                return false;
        return true;
    }

    @Override
    public Boolean visit(MethodContract n, Node arg) {
        // TODO
        return false;
    }

    @Override
    public Boolean visit(StoreEverythingExpression n, Node arg) {
        return arg instanceof StoreEverythingExpression;
    }

    @Override
    public Boolean visit(ArrayRangeExpression n, Node arg) {
        if (arg instanceof ArrayRangeExpression) {
            ArrayRangeExpression m = (ArrayRangeExpression) arg;
            return nodeEquals(n.begin(), m.begin()) && nodeEquals(n.end(), m.end());
        }
        return false;
    }

    @Override
    public Boolean visit(AnyFieldExpression n, Node arg) {
        return arg instanceof AnyFieldExpression;
    }

    @Override
    public Boolean visit(StoreRefExpression n, Node arg) {
        // TODO not that easy, there might be aliasing?
        return false;
    }

    @Override
    public Boolean visit(InvariantDeclaration n, Node arg) {
        if (arg instanceof InvariantDeclaration)
            return nodeEquals(n.getExpression(), ((InvariantDeclaration) arg).getExpression());

        return false;
    }

    @Override
    public Boolean visit(JMLStatement n, Node arg) {
        if (arg instanceof JMLStatement) {
            JMLStatement m = (JMLStatement) arg;
            if (!m.getType().equals(n.getType()))
                return false;
            return nodeEquals(n.getArgument(), m.getArgument());
        }
        return false;
    }

    @Override
    public Boolean visit(QuantificationExpression n, Node arg) {
        if (arg instanceof QuantificationExpression) {
            QuantificationExpression q = (QuantificationExpression) arg;
            if (!n.getQuantifier().equals(q.getQuantifier()) || n.getTargets().size() != q.getTargets().size())
                return false;
            
            for (int i = 0; i < n.getTargets().size(); i++)
                if (!nodeEquals(n.getTargets().get(i), q.getTargets().get(i)))
                    return false;

            return nodeEquals(n.getRestriction(), q.getRestriction())
                    && nodeEquals(n.getExpression(), q.getExpression());
        }
        return false;
    }

    @Override
    public Boolean visit(TypeExpression n, Node arg) {
        if (arg instanceof TypeExpression) {
            TypeExpression n2 = (TypeExpression) arg;
            return nodeEquals(n.getExpr(), n2.getExpr()) && n.getOperator().equals(n2.getOperator())
                    && nodeEquals(n.getType(), n2.getType());
        }
        return false;
    }

    @Override
    public Boolean visit(TypeRelationExpression n, Node arg) {
        if (arg instanceof TypeRelationExpression) {
            TypeRelationExpression n2 = (TypeRelationExpression) arg;
            return nodeEquals(n.getLeft(), n2.getLeft()) && (n.isEquality() == n2.isEquality())
                    && nodeEquals(n.getRight(), n2.getRight());
        }
        return false;
    }
}
