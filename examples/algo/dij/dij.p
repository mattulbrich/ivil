...
program Dij source "dij.algo"
 ...
 loop1:
 sourceline 107
  skip LOOPINV ((nbors <: nbors0) & ((\forall r; ((r :: visited) -> ($load(distance, r) = $load(old_distance, r)))) & ((old_dom_distance <: dom_distance) & ((\forall s; ((s :: old_dom_distance) -> ($load(distance, s) <= $load(old_distance, s)))) & (($load(distance, start) = 0) & ((start :: dom_distance) & ((\forall x; ((x :: dom_distance) -> ($load(distance, x) >= 0))) & ((\forall y; ((y :: (dom_distance \ singleton(start))) -> (\exists x; ((x :: visited) & ((pair(x, y) :: dom_weight) & ($load(distance, y) = ($load(distance, x) + weight(x, y)))))))) & ((\forall b; (((b :: (dom_distance \ nbors)) & (pair(n, b) :: dom_weight)) -> ($load(distance, b) <= ($load(distance, n) + weight(n, b))))) & ((\forall a; (\forall b; (((a :: visited) & (b :: (dom_distance \ visited))) -> ($load(distance, a) <= $load(distance, b))))) & (\forall p; (\forall q; (((p :: visited) & ((!(q) :: nbors) & (pair(p, q) :: dom_weight))) -> (q :: dom_distance)))))))))))))), card(nbors)
 sourceline 72
  goto body1, after1
 body1:
