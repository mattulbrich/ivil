...
public final class InferencePath implements Comparable<InferencePath> {
    ...
    //
    // private InferencePath(List<Integer> path, UniversalType variable) throws
    // TypeSystemException {
    // this.path = path;
    ...
    // }
    //
    // UniversalType inferType(ASTElement node, final EnvironmentCreationState
    // state) throws TypeSystemException {
    //
    // assert (path.get(0) != -1);
    //
    ...
    // }
    //
    // static List<InferencePath> getPaths(UniversalType root, UniversalType
    // target) {
    ...
    // }
    //
    // private static void addChildren(UniversalType root, final UniversalType
    // target, LinkedList<Integer> path,
    ...
    public int compareTo(InferencePath p) {
        return 0;
        // // its impossible, that one path is a prefix of another, so its safe
        // to
